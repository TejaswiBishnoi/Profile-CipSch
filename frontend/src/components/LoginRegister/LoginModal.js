...
function LoginModal(props){
    ...
    function signIn(){
        setWarning(false);
        axios.post('http://localhost:5000/api/login', {email: email, pword: pass}).then(res=>{
            if (res.status == 200){
            ...
            if (e.response.status == 401) setWarning(true);
        ...
        >
            <Paper style={style} elevation={10} sx={{ paddingY:5, paddingX: 4, paddingTop: 2, background: theme.mainTextBack}}>
                <Stack direction={'column'} alignItems={'center'} width={'100%'}>
                    ...
                    <Typography display={warning?"":"none"} mt={6} fontWeight={550} color={theme.palette.orange.main} align="left" width={'80%'} fontSize={'14px'}>Invalid username or password</Typography>
                    <Box mt={warning?2:6} sx={{background: theme.customMainContainer.background, borderRadius: '13px'}} height={'45px'} width={'80%'}>
                        <Stack className={"LoginField " + theme.palette.mode + '-text'} height={'100%'} width={'100%'} direction={'row'} justifyContent={'space-between'} alignItems={'center'}>
                            <TextField sx={{width:'100%'}} value={email} onChange={(e)=>{setEmail(e.target.value)}} variant="outlined" placeholder="Email ID"></TextField>
                        </Stack>
                    </Box>
                    <Box mt={2} sx={{background: theme.customMainContainer.background, borderRadius: '13px'}} height={'45px'} width={'80%'}>
                        <Stack className={"LoginField " + theme.palette.mode + '-text'} height={'100%'} width={'100%'} direction={'row'} justifyContent={'space-between'} alignItems={'center'}>
                            <TextField sx={{width:'100%'}} type={show?"":"password"} onChange={(e)=>{setPass(e.target.value)}} value={pass} variant="outlined" placeholder="Password"></TextField>
                    ...
                    <Button onClick={signIn} disableElevation disableRipple sx={{width: '80%', mt: 4, height:'45px', borderRadius:'13px'}} variant="contained" color="orange">
                    ...
                    <Stack mt={4} width={'80%'} direction={'row'} justifyContent={'center'} alignItems={'center'}>
                        ...
                        Don't have an Account? </Typography><Button onClick={()=>{navigate('/auth/signup')}} sx={{p:0, ml:1}} color="orange" variant="text"><Typography textTransform={'none'} sx={{p: 0}} color={theme.palette.orange.main}>Get Started</Typography></Button>                
                    </Stack>
                </Stack>
            </Paper>
        </Modal>
